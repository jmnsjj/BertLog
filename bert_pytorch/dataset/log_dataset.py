...
class LogDataset(Dataset):
    ...
    def collate_fn(self, batch, percentile=100, dynamical_pad=True):
        ...
        # 应对 torch 的警告，output["time_input"]是一个list，里面每个元素都是ndarray，这里先将他整体转换为ndarray，速度会快很多
        output["time_input"] = torch.tensor(np.array(output["time_input"]), dtype=torch.float).squeeze(-1)
